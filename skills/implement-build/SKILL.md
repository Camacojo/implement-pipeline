...
After this phase the tests are **frozen**: developers may fix setup or fixtures only with a written justification in `tests.md` and may never weaken an assertion.
...
- Sequence packages in dependency order; run independent packages in parallel (one Agent call each, same message), with `isolation: worktree` when they share a repository.
- Then the orchestrator runs the **full** test suite and all quality gates of every affected repository (ground rule 9) — the same set CI runs, per the project docs; if the docs list fewer gates than the CI workflow, run what CI runs and fix the docs. Fix or delegate any failure. Nothing proceeds while something is red. Record the exact commands and counts.
