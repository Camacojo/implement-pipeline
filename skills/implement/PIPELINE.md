...
10. **Measure.** Write a timestamp (`HH:MM`) at every phase start and end in `state.md`, plus each agent's duration and token count from its completion notice. The final report includes the per-phase timing table.
