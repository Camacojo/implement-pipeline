...
12. **Never idle behind an agent whose output the next step does not need.** When a report arrives, act on it: a fix brief goes out when the reviewers of that layer have reported, a test author is briefed when the contracts are final, the verifier when the gates are green — regardless of what else is still running. The only things worth waiting for are inputs of the next action. Before the orchestrator waits, it names in one line what it is waiting for and which action needs it; if it cannot, it is not waiting, it is stalling. (Measured: 26 minutes of a 129-minute run were spent holding two fix briefs for a script package that no fix needed.)
